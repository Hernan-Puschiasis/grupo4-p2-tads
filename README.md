...
# Carga de datos
Para la carga de datos se utilizaron para guardar en el main las casas de cerveza, las cervezas, los estilos, las reseñas y los usuarios Hashs cerrados. Esto se debe a que la inserción y búsqueda en este TAD es O(1) promedio.  
...
Mientras se van leyendo las líneas, se lee cada dato y se crea cada clase, parseando a Long o Double los atributos correspondientes y teniendo en cuenta que si la abreviación de la cerveza es null, se descarta toda la línea. No se descarta línea en caso de que algún atributo sea "" como pasa con los usuarios por ejemplo.  
...
El promedio de tiempo de la consulta en 10 repeticiones es: 1126ms.  
A continuación se muestra el uso de memoria antes y después de aplicar el garbage collector.
...
El heap solo se ordena por reviews, en caso de empatar en reviews, se toma aleatoriamente el mayor (ocurre que hay empate en el último puesto).  
